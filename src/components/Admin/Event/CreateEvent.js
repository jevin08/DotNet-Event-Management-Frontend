...
const CreateEvent = () => {
    ...
    function validateForm() {
        return state.Name.length > 2  && state.Venue.length>5 && state.Speaker != null;
    }

    const updateState = (par) => {
        setState({ ...state, ...par });
    }
    ...
    const handleSubmit = (event) => {
        ...
        setIsLoading(true);
        axios.post(`https://localhost:7283/api/Event/Add`,
            state,
            {
                headers: {
            ...
            .then(response => {
                ...
                console.dir(evnt);
                let dely = Notify({message: `${evnt.Name} is added successfully.`,
                type: "success"});
                setIsLoading(true);
                setTimeout(()=>{
                    setIsLoading(false);
        ...
        <>
            <form onSubmit={handleSubmit} className="myForm border border-primary rounded">
                ...
                <div className="form-group">
                    ...
                    <input type="text" className="form-control" id="description" aria-describedby="descriptionHelp" placeholder="Enter description" value={state.Description} onChange={(e) => updateState({ Description: e.target.value })} />
                ...
                <div className="form-group">
                    ...
                    <input type="datetime-local" className="form-control" id="starttime" aria-describedby="starttimeHelp" placeholder="Enter start time" value={state.StartTime} onChange={(e) => updateState({ StartTime: e.target.value })} required />
                ...
                <div className="form-group">
                    ...
                    <input type="datetime-local" className="form-control" id="endtime" aria-describedby="endtimeHelp" placeholder="Enter end time" min={state.StartTime} value={state.EndTime} onChange={(e) => updateState({ EndTime: e.target.value })} required />
                    <small id="endtimeHelp" className="form-text text-muted">Ending time of an event.</small>
                </div>
                <div className="form-group">
                    <label labelfor="venue">Venue</label>
                    <input type="text" className="form-control" id="venue" aria-describedby="venueHelp" placeholder="Enter address" value={state.Venue} onChange={(e) => updateState({ Venue: e.target.value })} required />
                ...
                <div className="form-group">
                    ...
                    <input type="text" className="form-control" id="firstname" aria-describedby="firstnameHelp" placeholder="Enter first name" value={state.Speaker.firstname} onChange={(e) => updateUser({ firstname: e.target.value })} required />
                    <small id="firstnameHelp" className="form-text text-muted">Atleast 3 character.</small>
                </div>
                <div className="form-group">
                    <label labelfor="surname">Surname</label>
                    <input type="text" className="form-control" id="surname" aria-describedby="lastnameHelp" placeholder="Enter surname" value={state.Speaker.surname} onChange={(e) => updateUser({ surname: e.target.value })} required />
                    <small id="lastnameHelp" className="form-text text-muted">Atleast 3 character.</small>
                </div>
                <div className="form-group">
                    <label labelfor="email">Email address</label>
                    <input type="email" className="form-control" id="email" aria-describedby="emailHelp" placeholder="Enter email" value={state.Speaker.email} onChange={(e) => updateUser({ email: e.target.value })} required />
                ...
                <div className="form-group">
                    ...
                    <input type="text" className="form-control" id="studylevel" aria-describedby="studyLevelHelp" placeholder="Enter level of study" value={state.Speaker.studyLevel} onChange={(e) => updateUser({ studyLevel: e.target.value })} required />
                    <small id="studyLevelHelp" className="form-text text-muted">Ph.D</small>
                </div>

                <button type="submit" className="btn btn-primary btn-block mb-3" disabled={!validateForm()}>{isLoading ? <div className="spinner-border" role="status"></div> : 'Add Event'}</button>
            </form>
        </>
    );
}
